---
name: skl-003-status-and-handoff-validation
description: "Use when a workflow requires status & handoff validation, particularly for orchestrators, qa, or archivists."
---
...
**Common role families:** Orchestrators, QA, archivists, release packagers.

## Overview

Validates lifecycle state, output completeness, provenance, next owner, and blockers before handoff. This is a narrow, independently testable capability. It must be installed inside a role contract that defines authority, approved sources, integrations, and human approval boundaries.

## When to Use

Use this capability when a role needs status & handoff validation and the required input, expected output, and acceptance test can be stated before execution. Do not use it to smuggle broader decision rights or hidden tool access into a role.
...
| Transformation | Validates lifecycle state, output completeness, provenance, next owner, and blockers before handoff. |
...
3. Perform only this transformation: Validates lifecycle state, output completeness, provenance, next owner, and blockers before handoff.
...
- **Minimum fixture:** Handoff with missing owner/provenance and expected block.
