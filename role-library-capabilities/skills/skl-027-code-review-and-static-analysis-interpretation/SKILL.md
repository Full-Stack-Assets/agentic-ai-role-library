---
name: skl-027-code-review-and-static-analysis-interpretation
description: "Use when a workflow requires code review & static-analysis interpretation, particularly for code review, security, or qa."
---

# Code Review & Static-Analysis Interpretation

**Skill ID:** `SKL-027`  
**Primary category:** Evaluation  
**Common role families:** Code review, security, QA.

## Overview

Identifies correctness, maintainability, test, security, dependency, and policy risks in change sets. This is a narrow, independently testable capability. It must be installed inside a role contract that defines authority, approved sources, integrations, and human approval boundaries.

## When to Use

Use this capability when a role needs code review & static-analysis interpretation and the required input, expected output, and acceptance test can be stated before execution. Do not use it to smuggle broader decision rights or hidden tool access into a role.
...
| Transformation | Identifies correctness, maintainability, test, security, dependency, and policy risks in change sets. |
...
3. Perform only this transformation: Identifies correctness, maintainability, test, security, dependency, and policy risks in change sets.
...
- **Minimum fixture:** Diff with tests passing but an authorization flaw.
