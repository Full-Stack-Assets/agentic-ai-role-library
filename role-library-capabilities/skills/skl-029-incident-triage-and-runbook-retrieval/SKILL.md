---
name: skl-029-incident-triage-and-runbook-retrieval
description: "Use when a workflow requires incident triage & runbook retrieval, particularly for sre, security, or support."
---

# Incident Triage & Runbook Retrieval

**Skill ID:** `SKL-029`  
**Primary category:** Evaluation  
**Common role families:** SRE, security, support, field operations.

## Overview

Correlates approved telemetry, assesses scope/severity by policy, retrieves runbooks, and routes escalation. This is a narrow, independently testable capability. It must be installed inside a role contract that defines authority, approved sources, integrations, and human approval boundaries.

## When to Use

Use this capability when a role needs incident triage & runbook retrieval and the required input, expected output, and acceptance test can be stated before execution. Do not use it to smuggle broader decision rights or hidden tool access into a role.
...
| Transformation | Correlates approved telemetry, assesses scope/severity by policy, retrieves runbooks, and routes escalation. |
...
3. Perform only this transformation: Correlates approved telemetry, assesses scope/severity by policy, retrieves runbooks, and routes escalation.
...
- **Minimum fixture:** Alert storm with one safety/security escalation indicator.
