---
name: skl-030-threat-modeling-and-security-finding-triage
description: "Use when a workflow requires threat modeling & security finding triage, particularly for security, architecture, or privacy."
---

# Threat Modeling & Security Finding Triage

**Skill ID:** `SKL-030`  
**Primary category:** Evaluation  
**Common role families:** Security, architecture, privacy.

## Overview

Identifies attack surfaces, threat scenarios, compensating controls, evidence, and remediation priority. This is a narrow, independently testable capability. It must be installed inside a role contract that defines authority, approved sources, integrations, and human approval boundaries.

## When to Use

Use this capability when a role needs threat modeling & security finding triage and the required input, expected output, and acceptance test can be stated before execution. Do not use it to smuggle broader decision rights or hidden tool access into a role.
...
| Transformation | Identifies attack surfaces, threat scenarios, compensating controls, evidence, and remediation priority. |
...
3. Perform only this transformation: Identifies attack surfaces, threat scenarios, compensating controls, evidence, and remediation priority.
...
- **Minimum fixture:** Design with incomplete trust-boundary information.
