---
name: skl-031-data-quality-profiling-and-drift-detection
description: "Use when a workflow requires data quality profiling & drift detection, particularly for data quality, mlops, or analytics."
---

# Data Quality Profiling & Drift Detection

**Skill ID:** `SKL-031`  
**Primary category:** Analysis  
**Common role families:** Data quality, MLOps, analytics.

## Overview

Measures completeness, validity, freshness, duplicate rates, distributions, and schema drift without silently editing data. This is a narrow, independently testable capability. It must be installed inside a role contract that defines authority, approved sources, integrations, and human approval boundaries.

## When to Use

Use this capability when a role needs data quality profiling & drift detection and the required input, expected output, and acceptance test can be stated before execution. Do not use it to smuggle broader decision rights or hidden tool access into a role.
...
| Transformation | Measures completeness, validity, freshness, duplicate rates, distributions, and schema drift without silently editing data. |
...
3. Perform only this transformation: Measures completeness, validity, freshness, duplicate rates, distributions, and schema drift without silently editing data.
...
- **Minimum fixture:** Dataset with schema change and quality regression.
