---
name: skl-040-release-delivery-package-validation
description: "Use when a workflow requires release/delivery package validation, particularly for release packaging, devops, or document control."
---

# Release/Delivery Package Validation

**Skill ID:** `SKL-040`  
**Primary category:** Evaluation  
**Common role families:** Release packaging, DevOps, document control.

## Overview

Verifies final assets, metadata, format, checksums, approvals, evidence, and delivery status against a declared checklist. This is a narrow, independently testable capability. It must be installed inside a role contract that defines authority, approved sources, integrations, and human approval boundaries.

## When to Use

Use this capability when a role needs release/delivery package validation and the required input, expected output, and acceptance test can be stated before execution. Do not use it to smuggle broader decision rights or hidden tool access into a role.
...
| Transformation | Verifies final assets, metadata, format, checksums, approvals, evidence, and delivery status against a declared checklist. |
...
3. Perform only this transformation: Verifies final assets, metadata, format, checksums, approvals, evidence, and delivery status against a declared checklist.
...
- **Minimum fixture:** Package with correct files but a missing approval and mismatch label.
