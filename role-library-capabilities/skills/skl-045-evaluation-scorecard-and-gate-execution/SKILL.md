---
name: skl-045-evaluation-scorecard-and-gate-execution
description: "Use when a workflow requires evaluation scorecard & gate execution, particularly for qa, governance, or release."
---

# Evaluation Scorecard & Gate Execution

**Skill ID:** `SKL-045`  
**Primary category:** Evaluation  
**Common role families:** QA, governance, release, MLOps.

## Overview

Applies rubric-based quality, policy, technical, and completeness checks to produce reproducible pass/fail evidence. This is a narrow, independently testable capability. It must be installed inside a role contract that defines authority, approved sources, integrations, and human approval boundaries.

## When to Use

Use this capability when a role needs evaluation scorecard & gate execution and the required input, expected output, and acceptance test can be stated before execution. Do not use it to smuggle broader decision rights or hidden tool access into a role.
...
| Transformation | Applies rubric-based quality, policy, technical, and completeness checks to produce reproducible pass/fail evidence. |
...
3. Perform only this transformation: Applies rubric-based quality, policy, technical, and completeness checks to produce reproducible pass/fail evidence.
...
- **Minimum fixture:** Artifact that passes quality but fails provenance/approval evidence.
